...
public interface RoomsRespository extends CrudRepository<Rooms, Long>{
	...
	/*@Query(value="SELECT ro from Rooms ro "
			+ "where ro.idRoom not in("
			+ "select ro.idRoom "
			+ "from Rooms ro inner join ro.reservation r" 
			+ "where (r.startDate <= :start_date and r.finishDate = >= :finish_date)"
			+ "or (r.startDate < :finish_date and r.finishDate >= :finish_date)"
			+ "or (:start_date <= r.startDate and :finish_date >= r.finishDate)"
			+ ")")*/
	@Query(value="select * \n" + 
			"	from rooms ro\n" + 
